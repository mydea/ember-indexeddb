...
module('Acceptance | example', function (hooks) {
  ...
  test('example app works', async function (assert) {
    ...
    assert.deepEqual(
      ...
      [dateString1, dateString1, dateString1],
    ...
    assert
      .dom('[data-test-row]:nth-child(1) [data-test-is-read]')
      .hasText('true');
  ...
  test('model-bulk-save mixin works', async function (assert) {
    ...
    assert.equal(
      ...
      'state of item 1 is correct before saving'
    );
    assert.equal(
      item2.currentState.stateName,
      'root.loaded.created.uncommitted',
      'state of item 2 is correct before saving'
    );
    assert.deepEqual(
      Object.keys(item.changedAttributes()),
      ['title'],
      'changedAttributes of item 1 are correct before saving'
    );
    assert.deepEqual(
      Object.keys(item2.changedAttributes()),
      ['title'],
    ...
    assert.equal(
      item.currentState.stateName,
      'root.loaded.saved',
      'state of item 1 is correct after saving'
    );
    assert.equal(
      item2.currentState.stateName,
      'root.loaded.saved',
      'state of item 2 is correct after saving'
    );
    assert.deepEqual(
      Object.keys(item.changedAttributes()),
      [],
      'changedAttributes of item 1 are correct after saving'
    );
    assert.deepEqual(
      Object.keys(item2.changedAttributes()),
      [],
      'changedAttributes of item 2 are correct after saving'
    );
